...
public class SysParamServiceImpl extends ServiceImpl<SysParamMapper, SysParam> implements SysParamService {
    ...
    public IPage<SysParam> findPage(Page page, SysParamQueryParam sysParamQueryParam) throws ApiException {
        ...
        try {
            ...
            queryWrapper.lambda().orderByAsc(SysParam::getSerial);
    ...
    public boolean add(SysParam sysParam) throws ApiException {
        try {
            ...
            queryWrapper.eq("config_key",sysParam.getConfigKey());
            SysParam sysParam1 = sysParamMapper.selectOne(queryWrapper);
            if(sysParam1!=null){
                throw new ApiException("配置键已存在",HttpStatus.BAD_REQUEST);
            }
