<template>
  <Form ref="form" :model="formValidate" :rules="ruleValidate" :label-width="90">
    <FormItem label="名称" prop="name">
      <Input v-model="formValidate.name" placeholder="请输入名称" clearable></Input>
    </FormItem>
    <FormItem label="配置键" prop="configKey">
      <Input v-model="formValidate.configKey" placeholder="请输入配置键" clearable></Input>
    </FormItem>
    <FormItem label="配置值" prop="configValue">
      <Input v-model="formValidate.configValue" placeholder="请输入配置值" clearable></Input>
    </FormItem>
    ...
    return {
      ...
      ruleValidate: {
        ...
        name: [
          ...
          {
            validator: (rule, value, callback) => {
              this.$rule.ruleLength(rule, value, callback, 100)
            },
            trigger: 'blur'
          }
        ],
        configKey: [
          ...
          {
            validator: (rule, value, callback) => {
              this.$rule.ruleNoChinese(rule, value, callback, 100)
            },
            trigger: 'blur'
          }
        ],
