...
<script>
import { columns, seek, params, modalParams, title } from './SystemParamPage/params'
...
export default {
  ...
  components: {
    addOrEdit: require('./SystemParamPage/addOrEdit').default
  },
  created () {
  },
  methods: {
    ...
    dataCallBack (params) {
      return this.api.getSystemParam(params).then(res => {
        return {
     ...
     */
    handleDelete (item) {
      this.api.delSystemParam({
        id: item.id
      }).then(() => {
        this.$Message.success(`删除系统参数${item.name}成功！`)
        this.setCount()
      })
    },
    handleSubmit () {
      return this.$refs[this.modalParams.type].handleSubmit().then(res => {
        ...
        if (type === 'add') {
          return this.api.addSystemParam(res).then(data => {
            this.$Message.success(`新增系统参数${res.name}成功！`)
            this.setCount()
          ...
          return this.api.updateSystemParam(res).then(data => {
            this.$Message.success(`修改系统参数${res.name}成功！`)
            this.setCount()
            return Promise.resolve(data)
          }).catch(err => {
            return Promise.reject(err)
