<template>
  <Form ref="form" :model="formValidate" :rules="ruleValidate" :label-width="90">
    <FormItem label="名称" prop="villageName">
      <Input v-model="formValidate.villageName" placeholder="请输入村庄名称" clearable></Input>
    </FormItem>
    <FormItem label="村落编号" prop="sn">
      <Input v-model="formValidate.sn" placeholder="请输入村落编号" clearable></Input>
    </FormItem>
    ...
    return {
      ...
      ruleValidate: {
        ...
        address: [
          ...
          {
            validator: (rule, value, callback) => {
              this.$rule.ruleLengthMin(rule, value, callback, [3, 200])
            },
            trigger: 'blur'
          }
        ],
        sn: [
          { required: true, message: '请输入村落编号', trigger: 'blur' },
          {
            validator: (rule, value, callback) => {
              this.$rule.ruleNumberMin(rule, value, callback, [6, 20])
            },
            trigger: 'blur'
          }
        ],
